...
public class SecurityConfig {
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOrigins(List.of("http://localhost:5432")); // e.g. react app running on port 5432
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.cors().and().authorizeHttpRequests()
                .requestMatchers("/api/filters", "/api/cars/check-car-available/**",
                        "/api/payments/check", "/api/cars/get-car-for-invoice/**",
                        "/api/payments/process-rental-payment", "/actuator/**")
                .permitAll()
                .requestMatchers("/api/**")
                .hasAnyRole("user")
                .anyRequest()
